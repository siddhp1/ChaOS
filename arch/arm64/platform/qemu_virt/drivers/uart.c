...
void uart_init(void) {
  ...
  UART_CR |= (1u << 0) | (1u << 8) | (1u << 9);  // Enable UART, RXE, TXE
}
...
void uart_irq_init(void) {
  ...
  UART_ICR = 0x7FF;  // Clear all pending UART interrupts
  UART_IMSC = UARTIMSC_RXIM |
              UARTIMSC_RTIM;  // Enable RX and receive timeout interrupts
}
